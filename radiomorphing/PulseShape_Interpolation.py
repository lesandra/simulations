...
def Interpolate_PulseShape(t1, trace1, x1, t2, trace2, x2, xdes, upsampling=None, zeroadding=None, ontrue=None, flow=10.e6, fhigh=1000.e6): #, nrdes):
    DISPLAY=0
    ...
    #print "Frequenz", fsample, xnew[1]-xnew[0]
    ...
    #print "Freq", len(freq), len(phi), len(xnew), len(t1), len(trace1), xnew[0], xnew[-1], (xnew[-1]-xnew[0])
    ...
    #print "Freq2", len(freq2), len(phi2), len(xnew2), len(t2),len(trace1), xnew2[0], xnew2[-1], (xnew2[-1]-xnew2[0])
...
####### inverse FFT for the signal at the desired position
    ...
    #print len(xnew), len(tracedes), xnew[0], xnew2[0]
